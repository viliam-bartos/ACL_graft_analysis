...
# HLAVNÍ ENGINE
# -------------------------------------------------------------------------
def process_single_volume(file_path, lat_classifier, model, device, run_viz_at_end=False):
    ...
    # 4. INFERENCE
    mask_arr = None
    if CONFIG["run_inference"] and model:
        logging.info("  -> Spouštím vnitřní multiclass inference (Blackwell).")
    ...
    if CONFIG["run_inference"]:
        try:
            ...
            if os.path.exists(CONFIG["model_ckpt"]):
                ...
                model.to(device)
                logging.info(f"Načten model Blackwell z: {CONFIG['model_ckpt']}")
            else:
                logging.warning(f"Váhy nenalezeny v {CONFIG['model_ckpt']} - Inference selže, model nemá detekci.")
                model = None
        except Exception as e:
             logging.error(f"Model Blackwell nelze inicializovat/načíst: {e}")
             model = None

    if CONFIG["mode"] == "FILE":
        file_path = CONFIG["input_path"]
        try:
            process_single_volume(file_path, lat_classifier, model, device, run_viz_at_end=True)
        except Exception as e:
        ...
        for f in files:
            try:
                process_single_volume(f, lat_classifier, model, device, run_viz_at_end=False)
            except Exception as e:
