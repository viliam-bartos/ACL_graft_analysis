    ...
    with torch.no_grad():
        for i, val_batch in enumerate(tqdm(val_loader, desc=f"Testing Fold {fold_idx}")):
            ...
            with autocast('cuda', dtype=torch.bfloat16):
                val_outputs = sliding_window_inference(val_images, roi_size=config['patch_size'], sw_batch_size=32, predictor=model, overlap=0.25, mode="gaussian")
...
def train_fold(config, train_files, val_files, fold_idx, run_dir, global_cv_csv_path):
    ...
    for epoch in range(config['epochs']):
        ...
        if (epoch + 1) % config['val_interval'] == 0:
            ...
            with torch.no_grad():
                for val_batch in tqdm(val_loader, desc=f"F{fold_idx} Ep {epoch + 1} [Val]", leave=False):
                    ...
                    inf_start = time.time()
                    with autocast('cuda', dtype=torch.bfloat16):
                        val_outputs = sliding_window_inference(inputs=val_images, roi_size=config['patch_size'], sw_batch_size=32, predictor=model, overlap=0.25, mode="gaussian")
            ...
            print(f"Ep {epoch + 1} | Val_Loss: {val_loss:.4f} | DICE [ACL: {v_d_acl:.4f}, Fem: {v_d_fem:.4f}, Tib: {v_d_tib:.4f}] | HD95 [ACL: {v_hd_acl:.2f}, Fem: {v_hd_fem:.2f}, Tib: {v_hd_tib:.2f}]")
        ...
        # Posun lr_scheduleru pro další epochu
        scheduler.step()
        ...
        # Early stopping is strictly disabled (1000 epochs forced)
